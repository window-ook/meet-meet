...
따라서 `useQuery`, `useInfiniteQuery`, `useMutation`이 핵심 도구로써 빈번히 사용될 거라 생각했고, <br>
...
- 디자인이 얼마나 직관적이었나요? → 평균 4.37037037
- 프로젝트의 전반적인 만족도는 어떠셨나요? → 평균 4.259259259
...
- 고정된 백엔드 API를 기반으로 진행하는 방식이라 한계로 인해 반영하지 못한 유저 피드백
- 더 좋은 코드 구조 생성
- 테스트 커버리지 향상 필요성
