...
`Lighthouse` 기준 이미지에 의한 LCP 유발로 성능 저하가 발생하여, 전체 페이지 평균 **83점** 정도의 성능 점수가 나왔습니다.
이미지 최적화를 진행하여 최종적으로 다음과 같은 결과를 얻었습니다.
...
아쉬운 점은 좀 더 높은 점수를 얻고 싶었는데 LCP에서 레거시 javascript 폴리필 이슈와,
css가 렌더링을 차단하는 문제를 해결하지 못했다는 점입니다.
